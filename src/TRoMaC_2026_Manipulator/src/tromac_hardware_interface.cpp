...
namespace tromac_hardware
{
  ...
  {
  public:
    // 2-joint (大pitch) 永久锁 0、不再参与控制；ROS 侧 5 个 active joint：
    // 索引 0..4 ⇄ 1-joint, 3-joint, 4-joint, 5-joint, 6-joint
    // 下位机 MCU 帧仍是 6 通道，CHANNEL_MAP 给出 ROS 索引 → MCU 通道 (0-indexed) 的映射，
    // MCU 通道 1 (Joint_2) 永久填 0
    static constexpr int CHANNEL_MAP[5] = {0, 2, 3, 4, 5};
        ...
        const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) override
    {
      ...
      // ROS 侧 5 个 joint 按 CHANNEL_MAP 散列到 MCU 6 通道，未映射的 channel 1 (Joint_2) 恒 0
      TRoMaC::VisionData vd{};
      int16_t ch[6] = {0, 0, 0, 0, 0, 0};
      double degs[NUM_JOINTS];
      for (int i = 0; i < NUM_JOINTS; ++i)
      {
        degs[i] = JOINT_DIR[i] * (hw_commands_[i] - JOINT_OFFSET_RAD[i]) * (180.0 / M_PI);
        ch[CHANNEL_MAP[i]] = static_cast<int16_t>(std::lround(degs[i] * DEG_SCALE));
      }
      vd.Joint_1 = ch[0];
      vd.Joint_2 = ch[1]; // 永久 0：2-joint 锁死
      vd.Joint_3 = ch[2];
      vd.Joint_4 = ch[3];
      vd.Joint_5 = ch[4];
      vd.Joint_6 = ch[5];
  ...
  private:
                       ...
                       const rclcpp::Logger &logger)
    {
      const bool button_rising = (rx.Button == 17 && prev_button_ != 17);
      {
        ...
        // MCU 6 通道里 Real_Joint_2 是被锁死的 2-joint 反馈，丢弃；其余按 CHANNEL_MAP 拉回
        const int16_t raw[6] = {
            rx.Real_Joint_1, rx.Real_Joint_2, rx.Real_Joint_3,
            rx.Real_Joint_4, rx.Real_Joint_5, rx.Real_Joint_6};
        for (int i = 0; i < NUM_JOINTS; ++i)
        {
          rx_joint_rad_[i] = JOINT_DIR[i] *
                                 static_cast<double>(raw[CHANNEL_MAP[i]]) * RAW_TO_RAD +
                             JOINT_OFFSET_RAD[i];
        }
        rx_data_valid_ = true;
      ...
      if (button_rising)
      {
        ...
        RCLCPP_INFO(logger,
                    "Btn=17 上升沿，snapshot=[%.3f %.3f %.3f %.3f %.3f] rad (1/3/4/5/6-joint)，"
                    "启动 1.2s force-sync 窗口",
                    snapshot_pos_[0], snapshot_pos_[1], snapshot_pos_[2],
                    snapshot_pos_[3], snapshot_pos_[4]);
      }
    ...
    void onReconnectedTriggerSync(const rclcpp::Logger &logger)
    {
      ...
      RCLCPP_INFO(logger,
                  "重连同步：snapshot=[%.3f %.3f %.3f %.3f %.3f] rad (1/3/4/5/6-joint)，"
    ...
    std::mutex rx_mutex_;
    double rx_joint_rad_[NUM_JOINTS]{};
    ...
    double snapshot_pos_[NUM_JOINTS]{};
